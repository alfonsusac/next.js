...
> - The `redirect` function works by throwing an error that will be caught by Next's routing system. So putting in event callbacks like `onClick` will not work
